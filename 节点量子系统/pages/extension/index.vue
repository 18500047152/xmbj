	...
	<view class="contentBox" :style="[{height:otherParams.height,'background-image':otherParams.backgroundImage}]">
		...
		<view class="urlBox" v-if="param == 'shop'">
			<view class="url" v-if="business_count > 0">
				...
				<text class="copy" @click="tuiGuang('shop')">推广赚{{tui_money}}元</text>
			</view>
			<view class="url" v-else>
				<text class="copy_alone" @click="tuiGuang('shop')">推广赚{{tui_money}}元</text>
			</view>
		</view>
		<!-- 从合伙人家跳过来的样式 -->
		<view class="urlBox" v-if="param == 'agent'">
			<view class="url" v-if="partner_count > 0">
				...
				<text class="copy" @click="tuiGuang('agent')">推广赚{{tui_money}}元</text>
			</view>
			<view class="url" v-else>
				<text class="copy_alone" @click="tuiGuang('agent')">推广赚{{tui_money}}元</text>
			</view>
		</view>
...
<script>
	...
	export default {
		...
		methods: {
			...
			/* canvas */
			reqCanvasQR(){ // 点击生成
				this.shareCanvasShow = true // 显示遮罩 如果fales canvas节点一起无
				let obj
				if(this.param == "shop"){
					//商家
					obj = {
                        // http://devqjd.yunduoke.cn/node/pages/list/shop?code=J16D92C&type=10
						url: this.info.content.level2_url+'?code='+this.info.code,
                        type:10,
					...
					//合伙人
					obj = {
                        // http://devqjd.yunduoke.cn/node/pages/list/agent?code=J16D92C&type=20
						url: this.info.content.level3_url+'?code='+this.info.code,
                        type:20,
						point_invite_code: this.code,
					}
				}
				...
				httpRequest_.post(url, obj).then(res => {
					this.getImageInfo(res.data).then(res => {
						...
						if(this.param == "shop"){
							//商家
							imgPath = "/static/shareapp1.jpg"
						}else if(this.param == "agent"){
							//合伙人
							imgPath = "/static/shareapp.jpg"
						}
						// 缓存完毕 开始绘画
						this.startCanvas(QRpath,imgPath)
